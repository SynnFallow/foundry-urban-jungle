...
export function ironclawRollActorChat(inputstring, speaker) {
    ...
    if (!speaker?.actor || !speaker?.token) {
        ui.notifications.warn(game.i18n.localize("urbanjungle.ui.actorNotSelectedActorroll"));
        return;
    }
    ...
    if (!actor) {
        ...
        if (!actor) {
            ui.notifications.warn(game.i18n.format("urbanjungle.chat.actorNotFoundForSpeaker", { "name": speaker.toString() }));
            return;
        }
    }
