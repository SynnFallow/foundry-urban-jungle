 ...
 * @extends {Item}
 */
export class urbanjungleItem extends Item {
    /** @override
    ...
    async sendInfoToChat() {
        ...
        const actorData = this.actor ? this.actor.data.data : {};
        const itemData = item.data;
        const confirmSend = game.settings.get("urbanjungle", "confirmItemInfo");
        ...
        switch (item.type) {
            case 'gift':
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.tags")}:</strong> ${itemData.giftTags}</p>
                        <p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.refresh")}:</strong> ${itemData.refresh}, <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.exhausted")}:</strong> 
                        ${itemData.exhaustWhenUsed ? (itemData.exhausted ? game.i18n.localize("urbanjungle.yes") : game.i18n.localize("urbanjungle.no")) : game.i18n.localize("urbanjungle.never")}</p>`;
                if (itemData.useDice) contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.giftDice")}:</strong> ${itemData.useDice},
                                                   <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.defaultTN")}:</strong> ${itemData.defaultTN}</p>`;
                else contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.giftDiceNothing")}</strong></p>`;
                break;
            case 'extraCareer':
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.name")}:</strong> ${itemData.careerName}</p>
                        <p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.dice")}:</strong> ${itemData.dice}</p>
                        <p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.skills")}:</strong> ${itemData.careerSkill1}, ${itemData.careerSkill2}, ${itemData.careerSkill3}</p>`;
                break;
            case 'weapon':
                if (itemData.hasResist)
                    contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.resistWith")}:</strong> ${itemData.specialResist} vs. 3</p>`;
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.effect")}:</strong> ${itemData.effect}</p>
                        <p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.descriptors")}:</strong> ${itemData.descriptors}</p>
                        <p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.equip")}:</strong> ${itemData.equip}, <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.range")}:</strong> ${itemData.range}</p>`;
                if (itemData.attackDice) contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.attackDice")}:</strong> ${itemData.attackDice}</p>`;
                if (itemData.useSpark) contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.sparkDice")}:</strong> ${itemData.sparkDie}</p>`;
                if (itemData.defenseDice) contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.parryDice")}:</strong> ${itemData.defenseDice}</p>`;
                if (itemData.counterDice) contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.counterDice")}:</strong> ${itemData.counterDice}</p>`;
                break;
            case 'illumination':
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.dimLight")}:</strong> ${itemData.dimLight}, <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.brightLight")}:</strong> ${itemData.brightLight},
                            <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.angle")}:</strong> ${itemData.lightAngle}</p>`;
                break;
            case 'armor':
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.armorDice")}:</strong> ${itemData.armorDice}, 
                            <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.worn")}:</strong> ${itemData.worn ? game.i18n.localize("urbanjungle.yes") : game.i18n.localize("urbanjungle.no")}</p>`;
                break;
            case 'shield':
                contents += `<p><strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.coverDie")}:</strong> ${itemData.coverDie}, 
                            <strong>${game.i18n.localize("urbanjungle.chatInfo.itemInfo.held")}:</strong> ${itemData.held ? game.i18n.localize("urbanjungle.yes") : game.i18n.localize("urbanjungle.no")}</p>`;
                break;
            default:
                break;
        }
        ...
        if (confirmSend) {
            let confirmed = false;
            let dlog = new Dialog({
                title: game.i18n.format("urbanjungle.dialog.chatInfo.itemInfo.title", { "name": this.data.name }),
                content: `
     <form>
      <h1>${game.i18n.format("urbanjungle.dialog.chatInfo.itemInfo.header", { "name": this.data.name })}</h1>
     </form>
     `,
                buttons: {
                    one: {
                        icon: '<i class="fas fa-check"></i>',
                        label: game.i18n.localize("urbanjungle.dialog.send"),
    ...
    automaticDamageCalculation(info, ignoreresist = false, onlyupdate = false) {
        if (!game.settings.get("urbanjungle", "calculateAttackEffects")) {
        ...
        if (!info.tnData) { // If the roll info is in highest mode, assume the attack was a counter-attack, and set the flags accordingly
            let updatedata = {
                flags: { "urbanjungle.hangingAttack": "counter", "urbanjungle.hangingWeapon": this.id, "urbanjungle.hangingActor": this.actor?.id, "urbanjungle.hangingToken": this.actor?.token?.id }
            };
        ...
        if (ignoreresist === false && itemData.hasResist) { // If the weapon's attack was a successful resist roll, set the flags accordingly and return out
            let updatedata = {
                flags: {
                    "urbanjungle.hangingAttack": "resist", "urbanjungle.hangingWeapon": this.id, "urbanjungle.hangingActor": this.actor?.id, "urbanjungle.hangingToken": this.actor?.token?.id,
                    "urbanjungle.resistSuccess": success, "urbanjungle.resistSuccessCount": usedsuccesses
                }
            };
            info.message.update(updatedata);
            ...
            let updatedata = {
                flags: {
                    "urbanjungle.hangingAttack": "attack", "urbanjungle.hangingWeapon": this.id, "urbanjungle.hangingActor": this.actor?.id, "urbanjungle.hangingToken": this.actor?.token?.id,
                    "urbanjungle.attackSuccess": success, "urbanjungle.attackSuccessCount": usedsuccesses
                }
            };
            info.message.update(updatedata);
        }
        ...
        if (onlyupdate) {
            ...
            if (game.settings.get("urbanjungle", "calculateDisplaysFailed")) {
    ...
    async resolveCounterAttack(message) {
        let info = await copyToRollTNDialog(message, "urbanjungle.dialog.counterResolve.title");
    ...
    resendNormalAttack(message) {
        const success = message.getFlag("urbanjungle", "attackSuccess");
    ...
    successfulAttackToChat(success, usedsuccesses) {
        ...
        <h3 class="chat-header-lesser">${game.i18n.format("urbanjungle.chatInfo.damageCalcInfo.header", { "name": item.name })}</h3>
        </header>
        <div class="chat-content"><div class="chat-item">`;

        if (success) {
            contents += `<p style="color:${CommonSystemInfo.resultColors.success}">${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.attackSuccess")}:</p>`;
        } else {
            contents += `<p style="color:${CommonSystemInfo.resultColors.tie}">${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.attackTied")}:</p>`;
        }

        if (itemData.effectsSplit.includes("slaying")) {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.slayingDamage")}: <strong>${itemData.damageEffect + (usedsuccesses * 2)}</strong></p>`;
        } else if (itemData.effectsSplit.includes("critical")) {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.criticalDamage")}: <strong>${itemData.damageEffect + Math.floor(usedsuccesses * 1.5)}</strong></p>`;
        } else {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.normalDamage")}: <strong>${itemData.damageEffect + usedsuccesses}</strong></p>`;
        }
        if (itemData.effectsSplit.includes("impaling")) {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.impalingDamage")}: <strong>${itemData.damageEffect + (usedsuccesses * 2)}</strong>, ${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.impalingNote")}</p>`;
        }

        if (itemData.effectsSplit.includes("penetrating")) {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.penetratingAttack")}</p>`;
        }
        if (itemData.effectsSplit.includes("weak")) {
            contents += `<p>${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.weakAttack")}</p>`;
        }
        contents += `<p class="small-text">${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.allEffects")}: ${itemData.effect}</p>`;
    ...
    failedAttackToChat() { // This function is mostly used for resist rolls to specifically note if the resistance check failed for the attacker
        ...
        if (itemData.hasResist) {
            contents += `<p style="color:${CommonSystemInfo.resultColors.failure}">${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.attackResisted")}</p>`;
        } else {
            contents += `<p style="color:${CommonSystemInfo.resultColors.failure}">${game.i18n.localize("urbanjungle.chatInfo.damageCalcInfo.attackFailed")}</p>`;
        }
    ...
    attackRoll() {
        ...
        const donotdisplay = game.settings.get("urbanjungle", "calculateDoesNotDisplay");
    ...
    sparkRoll() {
        ...
        rollHighestOneLine(data.sparkDie, game.i18n.localize("urbanjungle.dialog.sparkRoll.label"), "urbanjungle.dialog.sparkRoll.title", this.actor);
    }
        ...
        if (this.actor) {
            ...
            switch (rolltype) {
                case 0: // Generic gift roll
                    this.actor.popupSelectRolled(stats, tnyes, usedtn, "", formconstruction, (usesmoredice ? [diceid] : null), (usesmoredice ? [dicearray] : null), this.data.name + " " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.giftRoll") + (this.data.data.exhaustWhenUsed ? ", " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.gift") + " <strong>" + game.i18n.localize("urbanjungle.chatInfo.itemInfo.exhausted") + "</strong>" : ": "), callback);
                    break;
                case 1: // Parry roll
                    this.actor.popupDefenseRoll(stats, tnyes, usedtn, "", formconstruction, (usesmoredice ? [diceid] : null), (usesmoredice ? [dicearray] : null), this.data.name + " " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.parryRoll") + ": ", true, callback);
                    break;
                case 2: // Attack roll
                    this.actor.popupAttackRoll(stats, tnyes, usedtn, "", formconstruction, (usesmoredice ? [diceid] : null), (usesmoredice ? [dicearray] : null), this.data.name + " " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.attackRoll") + (this.data.data.effect ? ", " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.effect") + ": " + this.data.data.effect + (this.data.data.hasResist ? ", " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.resistWith") + " " + this.data.data.specialResist + " vs. 3 " : "") : ": "), callback);
                    break;
                case 3: // Counter roll
                    this.actor.popupCounterRoll(stats, tnyes, usedtn, "", formconstruction, (usesmoredice ? [diceid] : null), (usesmoredice ? [dicearray] : null), this.data.name + " " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.counterRoll") + (this.data.data.effect ? ", " + game.i18n.localize("urbanjungle.chatInfo.itemInfo.effect") + ": " + this.data.data.effect : ": "), callback);
                    break;
                default:
    ...
    popupExhaustGift() {
        ...
        let speaker = getMacroSpeaker(this.actor);
        let dlog = new Dialog({
            title: game.i18n.format("urbanjungle.dialog.exhaustGift.title", { "name": speaker.alias }),
            content: `
     <form>
      <h1>${game.i18n.format("urbanjungle.dialog.exhaustGift.header", { "item": this.data.name, "actor": this.actor?.data.name })}</h1>
     <div class="form-group">
       <label class="normal-label">Send to chat?</label>
       <input type="checkbox" id="send" name="send" value="1" checked></input>
     </div>
     ...
     `,
            buttons: {
                one: {
                    icon: '<i class="fas fa-check"></i>',
                    label: game.i18n.localize("urbanjungle.dialog.exhaust"),
                    callback: () => confirmed = true
                },
                two: {
                    icon: '<i class="fas fa-times"></i>',
            ...
            close: html => {
                if (confirmed) {
                    ...
                    if (sent) {
                        let contents = `<div class="urbanjungle"><header class="chat-item flexrow">
        <img class="item-image" src="${item.img}" title="${item.name}" width="20" height="20"/>
        <div class="chat-header-small">${game.i18n.format("urbanjungle.dialog.exhaustGift.chatMessage", { "name": item.name })}</div>
        </header>
        </div>`;
                        let chatData = {
                            "content": contents,
    ...
    popupWeaponRollType() {
	   ...
	   <div class="form-group">`;
        ...
        if (itemData.canAttack) {
            constructionstring += `<label>${game.i18n.localize("urbanjungle.dialog.weaponRoll.attack")}:</label>
	    <input type="radio" id="attack" name="weapon" value="0" ${first ? "" : "checked"}></input>`;
            first = first || "attack";
        }
        if (itemData.canSpark) {
            constructionstring += `<label>${game.i18n.localize("urbanjungle.dialog.weaponRoll.spark")}:</label>
	    <input type="radio" id="spark" name="weapon" value="1" ${first ? "" : "checked"}></input>`;
            first = first || "spark";
        }
        if (itemData.canDefend) {
            constructionstring += `<label>${game.i18n.localize("urbanjungle.dialog.weaponRoll.parry")}:</label>
	    <input type="radio" id="defend" name="weapon" value="2" ${first ? "" : "checked"}></input>`;
            first = first || "defend";
        }
        if (itemData.canCounter) {
            constructionstring += `<label>${game.i18n.localize("urbanjungle.dialog.weaponRoll.counter")}:</label>
        ...
        let speaker = getMacroSpeaker(this.actor);
        let dlog = new Dialog({
            title: game.i18n.format("urbanjungle.dialog.weaponRoll.title", { "name": speaker.alias }),
            content: `
     <form>
      <h1>${game.i18n.format("urbanjungle.dialog.weaponRoll.header", { "item": this.data.name, "actor": this.actor?.data.name })}</h1>
      ${constructionstring}
     </form>
     `,
            buttons: {
                one: {
                    icon: '<i class="fas fa-check"></i>',
                    label: game.i18n.localize("urbanjungle.dialog.pick"),
                    callback: () => confirmed = true
                },
                two: {
                    icon: '<i class="fas fa-times"></i>',
